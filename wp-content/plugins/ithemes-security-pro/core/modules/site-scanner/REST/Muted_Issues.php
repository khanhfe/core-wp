...
class Muted_Issues extends \WP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! $valid instanceof \WP_User ) {
			return new \WP_Error( 'rest_cannot_view', __( 'Sorry, you cannot view muted issues.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}
	...
	public function get_item( $request ) {
		if ( ! \ITSEC_Site_Scanner_Util::is_issue_muted( $request['id'] ) ) {
			return new \WP_Error( 'rest_not_found', __( 'This issue has not been muted.', 'it-l10n-ithemes-security-pro' ), array( 'status' => \WP_Http::NOT_FOUND ) );
		}
	...
	public function create_item_permissions_check( $request ) {
		...
		if ( ! $valid instanceof \WP_User ) {
			return new \WP_Error( 'rest_cannot_create', __( 'Sorry, you cannot mute issues.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}
	...
	public function create_item( $request ) {
		if ( \ITSEC_Site_Scanner_Util::is_issue_muted( $request['id'] ) ) {
			return new \WP_Error( 'rest_issue_already_muted', __( 'This issue has already been muted.', 'it-l10n-ithemes-security-pro' ), array( 'status' => \WP_Http::BAD_REQUEST ) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! $valid instanceof \WP_User ) {
			return new \WP_Error( 'rest_cannot_delete', __( 'Sorry, you cannot unmute issues.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}
	...
	public function delete_item( $request ) {
		if ( ! \ITSEC_Site_Scanner_Util::is_issue_muted( $request['id'] ) ) {
			return new \WP_Error( 'rest_not_found', __( 'This issue is not muted.', 'it-l10n-ithemes-security-pro' ), array( 'status' => \WP_Http::NOT_FOUND ) );
		}
	...
	protected function has_permission( \WP_REST_Request $request ) {
		...
		if ( $token = $this->get_token_from_auth_header( $request->get_header( 'Authorization' ) ) ) {
			$valid = \ITSEC_Site_Scanner_Util::validate_muted_issues_auth_token( $token );
	...
	protected function get_token_from_auth_header( $header ) {
		$prefix = 'Bearer ';
