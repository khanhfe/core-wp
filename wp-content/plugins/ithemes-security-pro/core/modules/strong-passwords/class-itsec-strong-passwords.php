...
final class ITSEC_Strong_Passwords implements Runnable {
	...
	public function register_group_setting( User_Groups\Settings_Registry $registry ) {
		if ( ITSEC_Lib_Password_Requirements::is_requirement_enabled( 'strength' ) ) {
			$registry->register( new User_Groups\Settings_Registration( 'password-requirements', 'requirement_settings.strength.group', User_Groups\Settings_Registration::T_MULTIPLE, static function () {
				return [
					'title'       => __( 'Require Strong Passwords', 'it-l10n-ithemes-security-pro' ),
					'description' => __( 'Force users in the group to use strong passwords.', 'it-l10n-ithemes-security-pro' ),
				];
			} ) );
		}
	}
	...
	public function reason( $evaluation ) {
		return esc_html__( 'Due to site rules, a strong password is required for your account. Please choose a new password that rates as strong on the meter.', 'it-l10n-ithemes-security-pro' );
	}
	...
	public function get_settings_config() {
		return array(
			'label'       => esc_html__( 'Strong Passwords', 'it-l10n-ithemes-security-pro' ),
			'description' => esc_html__( 'Force users to use strong passwords as rated by the WordPress password meter.', 'it-l10n-ithemes-security-pro' ),
	...
	public function sanitize_settings( $settings ) {
		return array(
			array( 'user-groups', 'group', esc_html__( 'User Groups for Strong Passwords', 'it-l10n-ithemes-security-pro' ) ),
		);
	}
