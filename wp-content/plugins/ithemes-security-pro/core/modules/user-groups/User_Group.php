...
final class User_Group implements Matchable {
	...
	public function get_description() {
		...
		if ( $roles = $this->get_roles() ) {
			$parts[] = wp_sprintf( __( 'the following roles: %l', 'it-l10n-ithemes-security-pro' ), array_map( 'translate_user_role', $roles ) );
		}

		if ( $canonical = $this->get_canonical_roles() ) {
			$parts[] = wp_sprintf( __( 'the following canonical roles: %l', 'it-l10n-ithemes-security-pro' ), array_map( 'translate_user_role', $canonical ) );
		}

		if ( $min_role = $this->get_min_role() ) {
			$parts[] = sprintf( __( 'roles greater than %s', 'it-l10n-ithemes-security-pro' ), translate_user_role( $min_role ) );
		}

		if ( $users = $this->get_users() ) {
			$parts[] = wp_sprintf( __( 'the following users: %l', 'it-l10n-ithemes-security-pro' ), wp_list_pluck( $users, 'user_login' ) );
		}

		if ( ! $parts ) {
			return wp_sprintf( __( 'This user group contains no users.', 'it-l10n-ithemes-security-pro' ) );
		}

		return wp_sprintf( __( 'This user group contains %l.', 'it-l10n-ithemes-security-pro' ), $parts );
	}
