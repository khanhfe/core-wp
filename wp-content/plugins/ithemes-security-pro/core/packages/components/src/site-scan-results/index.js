...
function SiteScanResults( { results, showSiteUrl = true, showErrorDetails = true, authContext = {} } ) {
	const siteUrl = results.url;

	return (
		<div className="itsec-site-scan-results">
			{ showSiteUrl && siteUrl && <h4>{ sprintf( __( 'Site: %s', 'it-l10n-ithemes-security-pro' ), siteUrl ) }</h4> }

			{ isWPError( results ) ? <WPErrorDetails results={ results } showErrorDetails={ showErrorDetails } /> : (
				<Fragment>
					<SystemErrorDetails results={ results } />
					<KnownVulnerabilities results={ results } authContext={ authContext } />
