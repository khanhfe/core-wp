...
class ITSEC_Login_Interstitial_Session {
	...
	public function verify( $user_id, $signature ) {
		if ( $this->is_expired() ) {
			return new WP_Error( 'itsec-lib-login-interstitial-verify-failed-session-expired', esc_html__( 'Session expired.', 'it-l10n-ithemes-security-pro' ) );
		}
		...
		if ( true !== $signature_verified ) {
			return new WP_Error( 'itsec-lib-login-interstitial-verify-failed-invalid-signature', esc_html__( 'Invalid signature.', 'it-l10n-ithemes-security-pro' ) );
		}

		if ( ! $user_id || $this->get_user()->ID !== $user_id ) {
			return new WP_Error( 'itsec-lib-login-interstitial-verify-failed-invalid-user', esc_html__( 'Invalid user.', 'it-l10n-ithemes-security-pro' ) );
		}
	...
	public function get_signature() {
		...
		if ( ! $hash ) {
			return new WP_Error( 'itsec-lib-login-interstitial-signature-failed', esc_html__( 'Could not calculate signature.', 'it-l10n-ithemes-security-pro' ) );
		}
	...
	public static function get( $id ) {
		...
		if (
			...
			return new WP_Error( 'itsec-lib-login-interstitial-not-found', esc_html__( 'Interstitial state not found.', 'it-l10n-ithemes-security-pro' ) );
		}
